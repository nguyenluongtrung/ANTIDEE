...
export const UpdatePostForum = ({ allTopics, selectedForumPost, onClose }) => {
	...
	const onSubmit = async (formData) => {
		const currentTitle = selectedForumPost?.title || '';
		const currentContent = selectedForumPost?.content || '';

		const updatedTitle =
			formData.title.trim() === '' ? currentTitle : formData.title;
		const updatedContent =
			formData.content.trim() === '' ? currentContent : formData.content;
		...
		if (imagesUrl || !formData.images) {
			try {
				...
				const result = await dispatch(
					updateForumPost({ forumPostData, postId: selectedForumPost._id })
				);
			...
			>
				...
				<div className="mt-3">
					...
					<div>
						<input
							{...register('title', {})}
							defaultValue={selectedForumPost?.title}
				...
				<div className="mt-3">
					...
					<div>
						<textarea
							{...register('content', {})}
							defaultValue={selectedForumPost?.content}
