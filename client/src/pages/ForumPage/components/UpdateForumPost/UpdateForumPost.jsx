...
export const UpdatePostForum = ({
  setIsOpenUpdatePostForum,
  chosenForumPostId,
}) => {
  const { isLoading: forumPostLoading, forumPosts } = useSelector(
    (state) => state.forumPosts
  );
  const [chosenForumPost, setChosenForumPost] = useState(null);
  ...
  useEffect(() => {
    if (!forumPosts || forumPosts.length === 0) {
      ...
      const foundPost = forumPosts.find(
        (post) => post._id === chosenForumPostId
      );
      setChosenForumPost(foundPost || null);
    }
  }, [dispatch, forumPosts, chosenForumPostId]);

  useEffect(() => {
    if (chosenForumPost && chosenForumPost.topic) {
      setSelectedTopics(chosenForumPost.topic.map((t) => t._id));
    }
  }, [chosenForumPost]);
  ...
  const onSubmit = async (formData) => {
    const currentTitle = chosenForumPost?.title || '';
  const currentContent = chosenForumPost?.content || '';

  const updatedTitle = formData.title.trim() === '' ? currentTitle : formData.title;
  const updatedContent = formData.content.trim() === '' ? currentContent : formData.content;

  if (updatedTitle.trim() === '' || updatedContent.trim() === '') {
    toast.error("Tiêu đề và Nội dung không được để trống", errorStyle);
    return;
  }
    if (imagesUrl || !formData.images) {
      try {
        ...
        const result = await dispatch(
          updateForumPost({ forumPostData, postId: chosenForumPostId })
        );
        if (result.type.endsWith("fulfilled")) {
          toast.success("Chỉnh sửa thành công", successStyle);
  ...
  useEffect(() => {
    if (chosenForumPost && chosenForumPost.topics) {
      setSelectedTopics(chosenForumPost.topics);
    }
  }, [chosenForumPost]);
    ...
    <div className="popup active">
      ...
      >
        ...
        <div className="mt-3">
          ...
          <div>
            <input
              ...
              defaultValue={chosenForumPost?.title}
        ...
        <div className="mt-3">
          ...
          <div>
            <textarea
              ...
              defaultValue={chosenForumPost?.content}
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <input
            defaultValue={chosenForumPost?.images}
            ref={fileRef}
