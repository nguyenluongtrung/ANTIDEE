...
import { deleteReply, getAllFeedbacks, replyFeedback, updateReply } from "../../../features/domesticHelperFeedback/domesticHelperFeedbackSlice";
...
export const ReplyFeedback = () => {
    ...
    const { account, isLoading: accountLoading } = useSelector((state) => state.auth)
    const { domesticHelperFeedbacks, isLoading } = useSelector((state) => state.domesticHelperFeedbacks);
    ...
    const handleCommentClick = () => {
        setReplyVisible((prevVisible) => !prevVisible);
    };
    ...
    const handleUpdateReply = async (e, feedbackId, replyId) => {
        e.preventDefault();
        const result = await dispatch(updateReply({ feedbackId, replyId, content: updatedContent }));
        if (result.type.endsWith('fulfilled')) {
                        ...
                        <div>
                            <div>
                                ...
                                <div className="flex">
                                    ...
                                    />
                                    <div>
                                        <p className="font-semibold">{feedback?.customerId?.name}</p>
                                        <p className="text-xs mb-1">{formatDateTime(feedback?.createdAt)}</p>
                                        <p className="ml-8 text-ellipsis break-words whitespace-pre-wrap">{feedback?.content}</p>
                                    </div>
                                </div>
                                <div className="flex justify-end text-sm gap-5" >
                                    <MdOutlineInsertComment className="w-6 h-6 hover:text-green cursor-pointer"
                                        onClick={() => handleCommentClick()} />
                                    <GoReply className="w-6 h-6 hover:text-green cursor-pointer"
                                                ...
                                                <div>
                                                    <div className="flex ">
                                                        <p className="font-semibold w-60">
                                                            {reply?.userId?.name} </p>
                                                        <HiOutlineDotsVertical className="justify-end"
                                                         ...
                                                         bg-white p-2 shadow-md rounded-lg
                                                          overflow-hidden">
                                                                <p className="hover:text-green"
                                                                    onClick={() => toggleEditReplyVisibility(reply?._id)}>Update</p>
                                                                <p className="hover:text-green"
                                                                    onClick={() => handleDeleteReply(feedback?._id, reply?._id)}>Delete</p>
                                                            </div>
                                                        )}
                                                    </div>


                                                    <p className="text-xs mb-1">{formatDateTime(reply?.createdAt)}</p>
                                                    ...
                                                    {showUpdateReply === reply?._id && (
                                                        <form onSubmit={(e) =>
                                                            ...
                                                            <div className="flex justify-end gap-3">
                                                                ...
                                                                <button type="submit" className="w-fit p-2 hover:text-green">
                                                                    Edit
                                                                </button>
                                                                <button className="w-fit p-2 hover:text-green"
                                                                    type="button"
                                                                    onClick={() =>
                            ...
                            {replyInputs[feedback._id] && (
                                <div className="mt-2">
                                    ...
                                    />
                                    <button className="hover:text-green flex justify-end"
