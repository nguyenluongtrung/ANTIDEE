...
const updateRatingDomesticHelper = asyncHandler(async (req, res) => {
  ...
  const feedbacks = await DomesticHelperFeedback.find({ domesticHelperId, feedbackFrom: "Khách hàng",});
...
const updateRatingCustomer = asyncHandler(async (req, res) => {
  ...
  const feedbacks = await DomesticHelperFeedback.find({ customerId, feedbackFrom: "Người giúp việc", });
  const numberOfRatings = feedbacks.length;
